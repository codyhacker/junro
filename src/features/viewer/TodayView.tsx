...
export function TodayView() {
  ...
  const initial = Math.max(0, timelines.findIndex(d => d.date === todayIso()))
  const [idx, setIdx] = useState(initial)

  if (!hydrated) return <div className="viewer-empty">Loading…</div>
  if (!trip || timelines.length === 0) return <div className="viewer-empty">No days to show yet.</div>

  const clamped = Math.min(idx, timelines.length - 1)
  const day = timelines[clamped]
  const tripDay = trip.days[clamped]
  const lodging = tripDay.lodgingId ? trip.lodgings.find(l => l.id === tripDay.lodgingId) : undefined
  ...
  return (
    <div className="viewer today">
      <header className="today-head">
        <a className="today-back" href={import.meta.env.BASE_URL}>←</a>
        <div className="today-daynav">
          <button disabled={clamped === 0} onClick={() => setIdx(clamped - 1)} aria-label="Previous day">‹</button>
          <span>
            <strong>Day {clamped + 1}</strong>
            <span className="today-date">{dayLabel(day.date)}</span>
          </span>
          <button disabled={clamped === timelines.length - 1} onClick={() => setIdx(clamped + 1)} aria-label="Next day">›</button>
        </div>
      </header>
      ...
      {day.stops.length === 0 ? (
        ...
        <>
          <ol className="today-stops">
            {day.stops.map((stop, i) => (
              <li key={stop.placeId} className="today-stop">
                <span className="today-time">{formatClock(stop.arrivalMin)}{stop.fixed && ' ●'}</span>
                <span className="today-stop-body">
                  <span className="today-stop-name">{CATEGORY_META[stop.category].emoji} {stop.name}</span>
                  {stop.address && <span className="today-stop-addr">{stop.address}</span>}
                </span>
                <a className="today-stop-map" href={mapsSearchUrl(stop.coord)} target="_blank" rel="noreferrer" aria-label={`Open ${stop.name} in Maps`}>◎</a>
          ...
          {routeCoords.length >= 2 && (
            <a className="today-directions" href={mapsDirectionsUrl(routeCoords, mode)} target="_blank" rel="noreferrer">
