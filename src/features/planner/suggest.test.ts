...
function cluster(id: string, placeIds: string[], centroid: [number, number], isExcursion = false): Cluster {
...
describe('suggestDays', () => {
  it('never assigns into a locked day, and keeps a neighborhood together', () => {
    const places = [
      place('a1', [2.33, 48.85]), place('a2', [2.331, 48.851]),
      place('b1', [2.37, 48.86]), place('b2', [2.371, 48.861]),
    ]
    const clusterResult: ClusterResult = {
      clusters: [cluster('cA', ['a1', 'a2'], [2.33, 48.85]), cluster('cB', ['b1', 'b2'], [2.37, 48.86])],
      excursions: [],
    }
    const days = [day('d1', '2026-10-05'), day('d2', '2026-10-06', { locked: true }), day('d3', '2026-10-07')]
  ...
  it('splits an oversized cluster so no day exceeds maxStopsPerDay', () => {
    ...
    const clusterResult: ClusterResult = {
      clusters: [cluster('big', places.map(p => p.id), [2.334, 48.8505])],
      excursions: [],
    }
    const days = [day('d1', '2026-10-05'), day('d2', '2026-10-06')]
    const { assignments, unplacedPlaceIds } = suggestDays({ clusterResult, days, places, lodgings: [HOTEL], prefs: PREFS })
  ...
  it('respects a must-see place\'s open days', () => {
    ...
    const clusterResult: ClusterResult = { clusters: [cluster('c', ['m1'], [2.33, 48.85])], excursions: [] }
    const { assignments } = suggestDays({ clusterResult, days, places: [museum], lodgings: [HOTEL], prefs: PREFS })
