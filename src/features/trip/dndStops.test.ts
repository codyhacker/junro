...
describe('computeStopDrop', () => {
  ...
  it('reorders within a day: drop c before a', () => {
    expect(computeStopDrop(days(), 'c', { kind: 'stop', placeId: 'a' })).toEqual({
      dayId: 'd1',
      placeIds: ['c', 'a', 'b'],
    })
  })
  ...
  it('reorders within a day: drop a before c', () => {
    expect(computeStopDrop(days(), 'a', { kind: 'stop', placeId: 'c' })).toEqual({
      dayId: 'd1',
      placeIds: ['b', 'a', 'c'],
    })
  })
  ...
  it('moves between days: drop a before y in d2', () => {
    expect(computeStopDrop(days(), 'a', { kind: 'stop', placeId: 'y' })).toEqual({
      dayId: 'd2',
      placeIds: ['x', 'a', 'y'],
    })
  })
  ...
  it('moves between days: drop onto the day appends', () => {
    expect(computeStopDrop(days(), 'a', { kind: 'day', dayId: 'd2' })).toEqual({
      dayId: 'd2',
      placeIds: ['x', 'y', 'a'],
    })
  })
