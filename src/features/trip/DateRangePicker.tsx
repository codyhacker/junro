...
const todayIso = () => { const d = new Date(); return isoOf(d.getFullYear(), d.getMonth(), d.getDate()) }
...
export function DateRangePicker({ start, end, onChange }: {
  ...
  if (pending) {
    if (hover) { lo = hover < pending ? hover : pending; hi = hover < pending ? pending : hover }
    else { lo = hi = pending }
  } else { lo = start; hi = end }

  return (
    <div className="cal">
      <div className="cal-head">
        <button className="cal-nav" aria-label="Previous month" onClick={() => setView(v => addMonths(v, -1))}>‹</button>
        <span className="cal-month">{monthLabel(view)}</span>
        <button className="cal-nav" aria-label="Next month" onClick={() => setView(v => addMonths(v, 1))}>›</button>
      </div>
      <div className="cal-weekdays">
        {WEEKDAYS.map((w, i) => <span key={i}>{w}</span>)}
      </div>
      <div className="cal-grid" onMouseLeave={() => setHover(null)}>
        {buildWeeks(view).flat().map((iso, i) => {
      ...
      <div className="cal-hint">
        {pending ? 'Pick the end date' : (start && end ? `${start} → ${end}` : 'Pick your start & end dates')}
      </div>
    </div>
  )
}
