...
export function AddPlace() {
  ...
  // Frame the collection while adding, instead of yanking to each result. The
  // box covers the local neighborhood or excursion pocket `center` belongs to
  // (see `focusBounds`) — not every saved place, so adding a Paris pin
  // doesn't pull Versailles into frame. Generous padding gives breathing room
  // and clears the planning panel. Cancel doesn't call this at all (see its
  // onClick below) — re-fitting to every place on cancel would reintroduce
  // the exact "zooms out to the whole trip" symptom this function exists to
  // avoid, one click after `pick()` correctly framed just the local area.
  function frame(center: [number, number]) {
    if (!trip) return
    const box = focusBounds(
      trip.places.map((p) => ({ id: p.id, coord: p.coord })),
        ...
        {pending && (
          <div className="add-place-confirm">
            ...
            <div className="add-place-actions">
              <button className="junro-secondary" onClick={done}>
                Cancel
              </button>
